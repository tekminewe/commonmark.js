...
var blockStarts = [
    ...
    function(parser, container) {
        ...
        var headerMatch = reTableRow.exec(parser.currentLine);
        if (!headerMatch) {
            return 0;
        }

        var delimiterCells = parseTableCells(delimiterMatch[0]);
        ...
        parser.addChild('table', parser.nextNonspace);
        parser.advanceNextNonspace();
    ...
    // table_row
    function(parser, container) {
        if (container.type !== 'table' && container.type !== 'table_row') {
            return 0;
        }
        ...
        parser.closeUnmatchedBlocks();
        parser.addChild('table_row', remaining.startsWith('|') ? 1 : 0);
        ...
        var cells = parseTableCells(remaining);
