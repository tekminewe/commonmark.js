...
var reMain = XRegExp.cache('^[\\s\\S]+?(?=[\\n`\\[\\]\\\\!<&*_\'"@:;xX~]|[a-z][a-z0-9.+-]{1,31}:|www\\d{0,3}\\.|(?:' + emailStartPattern + ')|$)');
...
var reUrl = XRegExp.cache('^(?:[a-z][\\pL\\d-]+:(?:\\/{1,3}|[\\pL\\d%])|www\\d{0,3}[.]|[\\pL\\d.\\-]+[.]\\pL{2,4}\\/)(?:\\[[\\da-f:]+\\]|[^\\s`!()\\[\\]{};:\'".,<>?«»“”‘’*_]|[*_]+(?=[^_*\\s])|[`!\\[\\]{};:\'",<>?«»“”‘’](?=[^\\s()<>])|\\.+(?=[\\pL\\d])|\\((?:[^\\s()<>]|(?:\\([^\\s()<>]+\\)))*\\))+', 'i');
var parseUrl = function(block) {
    ...
    if ((m = this.tryMatch(reUrl))) {
        ...
        if (this.pos === 0 || this.subject[this.pos - 1] === '_' || reNonWord.test(this.subject[this.pos - 1])) {
            ...
            node._destination = normalizeURI(m[0]);
            node._title = '';
...
var parseEmail = function(block) {
    ...
    if ((m = this.tryMatch(reEmail))) {
        ...
        if (this.subject.slice(this.pos).indexOf('@') !== -1) {
            console.log('failed to parse from `' + this.subject.slice(this.pos) + '`');
        }
        return false;
    }
}
