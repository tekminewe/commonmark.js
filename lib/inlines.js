...
var reMain = /^[\s\S]+?(?=[\n`\[\]\\!<&*_'"@:;xX]|[a-z][a-z0-9.+-]{1,31}:|www\d{0,3}\.|$)/;
...
var reUrl = /^(?:[a-z][\w-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s`!()\[\]{};:'".,<>?«»“”‘’*_]|[_*]+(?=[^_*\s])|[`!\[\]{};:'".,<>?«»“”‘’](?=[^\s()<>])|\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\))+/i;
var parseUrl = function(block) {
    ...
    var m;
    if ((m = this.match(reUrl))) {
        var node = new Node('link');
        node._destination = normalizeURI(m);
        node._title = '';
        node.appendChild(text(m));
        block.appendChild(node);
        return true;
    } else {
        return false;
...
var parseAtMention = function(block) {
    ...
    if ((m = this.tryMatch(reAtMention))) {
        // Only allow emoticons after non-word characters
...
var reEmoticon = /^(?:(:-?\))|(;-?\))|(:o)|(:-o)|(:-?])|(:-?d)|(x-d)|(:-?p)|(:-?[[@])|(:-?\()|(:['’]-?\()|(:-?\/)|(:-?s)|(:-?\|)|(:-?\$)|(:-x)|(<3|&lt;3)|(<\/3|&lt;\/3))(?=$|\s)/i;
var EMOTICONS = [
...
var parseEmoticon = function(block) {
    ...
    if ((m = this.tryMatch(reEmoticon))) {
        // Only allow emoticons after whitespace
        if (this.pos === 0 || reWhitespaceChar.test(this.subject[this.pos - 1])) {
            this.acceptMatch(m);
